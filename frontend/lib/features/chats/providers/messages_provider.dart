...
class LocalMessagesNotifier extends StateNotifier<List<Message>> {
  ...
  void setChatBeingViewed(bool isActive) {
    ...
    print('[LocalMessagesNotifier] 👁️ Chat $chatId viewer active: $_isViewerActive');
    
    // When viewer becomes active, mark all current unread messages as read
    if (isActive) {
      print('[LocalMessagesNotifier] 📖 Viewer activated - marking all unread messages as read');
      markAllUnreadAsRead();
    }
  }
  ...
  Future<void> markAllUnreadAsRead() async {
    final unreadMessages = state.where((m) => m.status != 'read' && m.senderId != currentUserId).toList();
    ...
    print('[LocalMessagesNotifier] 📋 Marking ${unreadMessages.length} unread messages as read');
  ...
  Future<void> initialize() async {
    ...
    try {
      print('[LocalMessagesNotifier] 🔄 Initializing messages for chat $chatId');
  ...
  void setupWebSocketListener(MessageWebSocketService webSocket) {
    print('[LocalMessagesNotifier] 🔌 Setting up WebSocket listener for chat $chatId');
  ...
  Future<void> _handleWebSocketEvent(WebSocketEvent event) async {
    try {
      ...
      if (event.chatId != chatId) {
        print('[LocalMessagesNotifier] ⏭️ Ignoring event for different chat: ${event.chatId} != $chatId');
        return;
      }
      ...
      print('[LocalMessagesNotifier] 📥 Received WebSocket event for chat $chatId (type: ${event.type})');
      ...
      if (event.type == WebSocketEventType.messageCreated) {
        try {
          ...
          if (!message.isDecrypted && message.encryptedContent.isNotEmpty) {
            try {
              final decryptedMessage = await MessageEncryptionService.decryptMessage(message);
              print('[LocalMessagesNotifier] 🔓 Decrypted message: ${message.id}');
              addMessage(decryptedMessage);
              
              // AUTO-READ: If new message from other user AND chat is being viewed, mark as read
              if (decryptedMessage.senderId != currentUserId && _isViewerActive) {
                print('[LocalMessagesNotifier] 🔵 Incoming message from other user while chat active: ${decryptedMessage.senderId}');
                print('[LocalMessagesNotifier] 🟨 Auto-marking as read: ${decryptedMessage.id}');
                markAsReadAndBroadcast(decryptedMessage.id, chatId);
              }
            } catch (decryptError) {
              print('[LocalMessagesNotifier] ❌ Decryption failed: $decryptError');
            ...
            if (message.senderId != currentUserId && _isViewerActive) {
              print('[LocalMessagesNotifier] 🔵 Incoming message from other user while chat active: ${message.senderId}');
              print('[LocalMessagesNotifier] 🟨 Auto-marking as read: ${message.id}');
      ...
      // Handle message status updates (sent, delivered, read)
        ...
        if (messageId != null && newStatus != null) {
          print('[LocalMessagesNotifier] 📨 Status update: $messageId → $newStatus');
  ...
  Future<void> loadMessagesFromServer() async {
    try {
      ...
      print('[LocalMessagesNotifier] 📥 Fetched ${messages.length} messages from server');
      
      // Decrypt messages
      final decryptedMessages = await MessageEncryptionService.decryptMessages(messages);
      ...
      if (state.isEmpty) {
        ...
        print('[LocalMessagesNotifier] ✅ Set initial ${decryptedMessages.length} messages');
      } else {
  ...
  void addMessage(Message message) {
    if (message.chatId == chatId) {
      ...
      if (!state.any((m) => m.id == message.id)) {
        ...
        if (state.length > 3) {
          print('[LocalMessagesNotifier]   Index 0: ${state[0].id} - ${state[0].createdAt}');
          print('[LocalMessagesNotifier]   Index 1: ${state[1].id} - ${state[1].createdAt}');
          print('[LocalMessagesNotifier]   Index ${state.length - 2}: ${state[state.length - 2].id} - ${state[state.length - 2].createdAt}');
          print('[LocalMessagesNotifier]   Index ${state.length - 1}: ${state[state.length - 1].id} - ${state[state.length - 1].createdAt}');
        }
        ...
        print('[LocalMessagesNotifier] ℹ️ Message already exists: ${message.id}');
      }
    }
  }
  ...
  void _markMessageAsDelivered(String messageId) {
    ...
    ChatApiService(baseUrl: 'http://localhost:8081')
        ...
        .catchError((e) {
          print('[LocalMessagesNotifier] ⚠️ Error marking $messageId as delivered: $e');
        });
  }
  ...
  void updateMessageStatus(String messageId, String newStatus) {
    ...
    if (index >= 0) {
      ...
      if (_isStatusUpgrade(currentStatus, newStatus)) {
        print('[LocalMessagesNotifier] 📦 Status upgrade: $messageId $currentStatus → $newStatus');
        final newList = [...state];
        // Use copyWith to preserve all fields and just update status
        newList[index] = updated.copyWith(status: newStatus);
        state = newList;
        print('[LocalMessagesNotifier] ✅ State updated for $messageId, new status: ${newList[index].status}');
      } else if (currentStatus == newStatus) {
        print('[LocalMessagesNotifier] ℹ️ Status already $currentStatus for $messageId, no change needed');
      } else {
        print('[LocalMessagesNotifier] ⏸️ Skipping downgrade: $currentStatus → $newStatus for $messageId (keeping $currentStatus)');
      }
    } else {
      print('[LocalMessagesNotifier] ⚠️ Message not found: $messageId (total messages: ${state.length})');
    }
  }
  ...
  Future<void> markAsReadAndBroadcast(String messageId, String chatId) async {
    try {
      print('[LocalMessagesNotifier] 🔴 markAsReadAndBroadcast called for $messageId');
      ...
      final apiService = ChatApiService(baseUrl: 'http://localhost:8081');
      print('[LocalMessagesNotifier] 📤 Calling API to mark $messageId as read and broadcast...');
      ...
      print('[LocalMessagesNotifier] ✅ API call success: $messageId marked as read');
    } catch (e) {
...
final localMessagesProvider =
    StateNotifierProvider.family<LocalMessagesNotifier, List<Message>,
        ({String chatId, String token, String currentUserId})>((ref, params) {
  ...
  notifier.initialize().then((_) {
    print('[LocalMessagesProvider] ✅ Notifier initialized for chat ${params.chatId}');
  }).catchError((e) {
...
final messagesWithCacheProvider =
  ...
  Future.delayed(Duration.zero, () {
    try {
      ...
      webSocket.eventStream.listen(
        (event) {
          print('[MessagesWithCacheProvider] 📨 WebSocket event: ${event.type.name} for chat ${event.chatId}');
          if (event.chatId == chatId &&
              (event.type == WebSocketEventType.messageCreated ||
               event.type == WebSocketEventType.messageReceived)) {
            print('[MessagesWithCacheProvider] 🔄 Invalidating cache for chat $chatId');
  ...
  try {
    print('[MessagesWithCacheProvider] 🔄 Fetching messages for chat $chatId...');
    final messages = await apiService.fetchMessages(
      token: token,
      chatId: chatId,
      limit: 50,
    ...
    print('[MessagesWithCacheProvider] ✓ Fetched and decrypted ${decryptedMessages.length} messages');
