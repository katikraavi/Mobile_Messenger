...
class ChatApiService {
  ...
  Future<List<Chat>> fetchChats({
    ...
    try {
      final url = Uri.parse(
        '$_baseUrl/api/chats?limit=$limit&offset=$offset',
      );

      print('[ChatApiService] 📡 Fetching chats from: $url');
      print('[ChatApiService] Token: ${token.isNotEmpty ? 'present' : 'EMPTY'}');
      ...
      if (response.statusCode == 200) {
        ...
        print('[ChatApiService] ❌ Failed to fetch chats: ${response.statusCode}');
  ...
  Future<List<Message>> fetchMessages({
    ...
    try {
      ...
      final uri = Uri.parse('$_baseUrl/api/chats/$chatId/messages')
  ...
  Future<bool> verifyChatParticipant({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to verify chat participation: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<Message> sendMessage({
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        throw Exception('Conflict: Message already sent (duplicate idempotency key)');
      } else if (response.statusCode >= 500) {
        throw Exception('Server error: ${response.statusCode}');
      } else {
  ...
  Future<List<Chat>> fetchArchivedChats({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to fetch archived chats: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<void> updateMessageStatus({
    ...
    try {
      ...
      print('[ChatApiService] 📤 Updating message status: $messageId → $newStatus');
      ...
      if (response.statusCode == 200) {
        print('[ChatApiService] ✅ Message status updated: $messageId → $newStatus');
      } else if (response.statusCode == 401) {
        throw Exception('Unauthorized: Invalid or expired token');
      } else if (response.statusCode == 404) {
        throw Exception('Message not found');
        ...
        print('[ChatApiService] ⚠️ Status update returned ${response.statusCode}');
